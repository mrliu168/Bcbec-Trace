...
func (t *EducationChaincode) queryComByCertNoAndName(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	queryString := fmt.Sprintf("{\"selector\":{\"docType\":\"%s\", \"primarykey\":\"%s\", \"Name\":\"%s\"}}", DOC_TYPE, primarykey, name)

	// 查询数据
	result, err := getComByQueryString(stub, queryString)
	if err != nil {
		return shim.Error("根据溯源编号及姓名查询信息时发生错误")
	}
	if result == nil {
		return shim.Error("根据指定的溯源编号及姓名没有查询到相关的信息")
	}
